...
--a) Cambio nombre de la función y acomodo las tabulaciones
not' b = case b of
        True -> False
        False -> True

--b) in :: [A] -> [A]  Cambio el nombre a la función
inn [x]         =  []
inn (x:xs)      =  x : inn xs
inn []          =  error "empty list"
{-
--c) Length :: [A] -> Int 
Length []        =  0
...
--d) Está mal escrita la construcción de la lista (1 : 2) : 3 : []
list123 = 1:(2:(3:[]))
